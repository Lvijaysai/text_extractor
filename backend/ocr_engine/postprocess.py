...
def parse_address(raw_address):
    ...
    addr_dict = {"full_address": clean_address, "area": "", "city": "", "state": "", "pin_code": "", "country": "INDIA"}
    ...
    if len(words) >= 1: addr_dict["state"] = words[-1]
    if len(words) >= 2: addr_dict["city"] = words[-2]
    if len(words) >= 3: addr_dict["area"] = words[-3]
